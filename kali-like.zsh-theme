...
configure_prompt() {
    ...
    if [[ $UID == 0 || $EUID == 0 ]]; then
        FGPROMPT="$FG[050]"    # Màu cyan sáng (root)
        WHITEPROMPT="$FG[255]" # Màu trắng (root)
    else
        WHITEPROMPT="$FG[255]" # Màu trắng (người dùng thường)
        FGPROMPT="$FG[050]"   # Màu cyan sáng (người dùng thường)
    fi
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'%{$WHITEPROMPT%}┌───(%{$FGPROMPT%}%B%n@%m%b%{$WHITEPROMPT%})(%{$WHITEPROMPT%}%B$(date +%Ih%Mp%S%P)%b%{$WHITEPROMPT%})%{$WHITEPROMPT%}[%{$reset_color%}%B%~%b%{$WHITEPROMPT%}]$(git_prompt_info)\n%{$WHITEPROMPT%}└─%{$FGPROMPT%}%B%(#.#.$)%b%{$reset_color%} '
            RPROMPT=
            ;;
        oneline)
            PROMPT=$'%B$FGPROMPT%n@%m%b%F{reset}:%B$WHITEPROMPT%~%b$(git_prompt_info)%F{reset}%(#.#.$) '
            RPROMPT=
            ;;
    esac
}
...
if [ "$USE_SYNTAX_HIGHLIGHTING" = yes ]; then
    ...
    if [ "$syntax_highlighting" = yes ]; then
        ...
        ZSH_HIGHLIGHT_STYLES[unknown-token]=fg=$CL_RED,bold
        ZSH_HIGHLIGHT_STYLES[reserved-word]=fg=white,bold
        ZSH_HIGHLIGHT_STYLES[suffix-alias]=fg=white
        ZSH_HIGHLIGHT_STYLES[global-alias]=fg=white
        ZSH_HIGHLIGHT_STYLES[precommand]=fg=white,bold
        ZSH_HIGHLIGHT_STYLES[commandseparator]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[autodirectory]=fg=white
        ZSH_HIGHLIGHT_STYLES[path]=fg=white
        ZSH_HIGHLIGHT_STYLES[path_pathseparator]=none
        ZSH_HIGHLIGHT_STYLES[path_prefix_pathseparator]=none
        ZSH_HIGHLIGHT_STYLES[globbing]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[history-expansion]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[command-substitution]=none
        ZSH_HIGHLIGHT_STYLES[command-substitution-delimiter]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[process-substitution]=none
        ZSH_HIGHLIGHT_STYLES[process-substitution-delimiter]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[single-hyphen-option]=fg=white
        ZSH_HIGHLIGHT_STYLES[double-hyphen-option]=fg=white
        ZSH_HIGHLIGHT_STYLES[back-quoted-argument]=none
        ZSH_HIGHLIGHT_STYLES[back-quoted-argument-delimiter]=fg=$CL_GREEN,bold
        ...
        ZSH_HIGHLIGHT_STYLES[rc-quote]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[dollar-double-quoted-argument]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[back-double-quoted-argument]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[back-dollar-quoted-argument]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[assign]=none
        ZSH_HIGHLIGHT_STYLES[redirection]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[comment]=fg=black,bold
        ZSH_HIGHLIGHT_STYLES[named-fd]=none
        ZSH_HIGHLIGHT_STYLES[numeric-fd]=none
        ZSH_HIGHLIGHT_STYLES[arg0]=fg=white,bold
        ZSH_HIGHLIGHT_STYLES[bracket-error]=fg=$CL_RED,bold
        ZSH_HIGHLIGHT_STYLES[bracket-level-1]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[bracket-level-2]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[bracket-level-3]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[bracket-level-4]=fg=$CL_GREEN,bold
        ZSH_HIGHLIGHT_STYLES[bracket-level-5]=fg=$CL_GREEN,bold
